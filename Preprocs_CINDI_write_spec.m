function Preprocs_CINDI_write_spec(yyyy, jjj_beg, jjj_end, instrument, loc, ...
...
%
% Used to rewrite spectra in required format, to be submitted for
...
% calibration file (from QDOAS)
calibration=load('CINDI2_spectra/cal_P1_2016_253_2.asc');
...
% %     for turr = 0:2,
    for turr = 1, % only used 600 gr/mm for CINDI2
        mode_vec = {'ZENITH', 'OFFAXIS', 'DIRECT SUN', 'M', 'X', 'HORIZON', 'ALMUCANTAR'};
        
        % only do off axis, add all data to that
        mode = 2;
        ...
        if ~isempty(ind),
            filename=['CINDI2_spectra/UToronto_MAXDOAS_36_VIS_spectra_CINDI2_201609' num2str(dd-1) '_v1.asc'];
%             filename=['UToronto_MAXDOAS_36_VIS_spectra_CINDI2_201609' num2str(dd-1) '_v1.asc'];
            write_files(filename, ind, line3_fin, line4_fin, line5_fin,...
                line6_fin, line7, sp_l1, sp_l2, ind_zenith, indhs, indas, mode_vec,...
                location, dd-1, mm, yyyy, calibration);
        end
...
function write_files(filename, ind,  line3_mat, line4_mat, line5_mat, line6_mat,...
    line7, sp_l1, sp_l2, ind_zenith, indhs, indas, mode_vec,location, dd, mm, yyyy, ...
    calibration)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
    ...
    fprintf(fid, '%s\n', ['# Total number of records = ' num2str(length(ind))]);
    ...
    for j = 1:length(ind)
        ...
        % all data after 11:40 on sept. 12 recorded with measurement mode=6
        % index of last actual horizon scan measurement is 463
        if dd==12
            last_hs_ind=find(line6_mat(:,2)==0);
            last_hs_ind=last_hs_ind+13;
            
            if i>last_hs_ind
                m_mode=mode_vec{2};
                if v_el==90, m_mode=mode_vec{1}; end
                if mod(v_el,1)~=0, m_mode=mode_vec{7}; end
            end
...
%         out=[calibration'; sp_l2(i,:)./t1int]; ???
        out=[calibration'; sp_l2(i,:)];
        fprintf(fid, '\t %.6f \t %.6f \n', out);
...
function [ind_out,  line3_out, line4_out, line5_out, line6_out, ...
    ...
    for ampm=0:1
        ...
        if ampm==0
            ...
            ind2=noon_elev_ind;
        elseif ampm==1
            ind1=noon_elev_ind+1;
            ind2=ind(end);
        end
        
        if binsize==1
            % for 1 degree bins, use SZA grid
            temp=min(line4_fin(ind1:ind2,1));
            min_elev=min([floor(temp),ceil(temp)]);
        elseif binsize<1
            % for small bins, just start from lowest elevation
