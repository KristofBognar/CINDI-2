%%% Program to write CINDI-2 data files for daily submission from our QDOAS
%%% files
...
% if day==256, 
%     error('.spe files were used for this day -- different format!!') 
% end
...
load('/home/kristof/work/GBS/PEARL-GBS/CINDI-2_2016/QDOAS_output/visible_cols.mat');

% read in QDOAS output file -- have to name that file manually!
file_nm= ['maxdoas_vis_' num2str(day) '.ASC'];
fid = fopen(file_nm, 'r');
fgetl(fid);
...
% filter out spectra before and after acquisition schedule 
% (use only 4:00 to 18:45 UTC)
ind= find(qdoas_raw(:, no2_vis.ft) >=4  & ...
    qdoas_raw(:, no2_vis.ft) < 18.75 );
qdoas_raw = qdoas_raw(ind,:);

% save QDOAS data in separate variable
save(['qdoas_' num2str(day) '.mat'],'qdoas_raw')

% filter out AS
% ind=find(...
%     qdoas_raw(:, no2_vis.elev) == 1 | ...
%     qdoas_raw(:, no2_vis.elev) == 2 | ...
%     qdoas_raw(:, no2_vis.elev) == 3 | ...
%     qdoas_raw(:, no2_vis.elev) == 4 | ...
%     qdoas_raw(:, no2_vis.elev) == 5 | ...
%     qdoas_raw(:, no2_vis.elev) == 6 | ...
%     qdoas_raw(:, no2_vis.elev) == 8 | ...
%     qdoas_raw(:, no2_vis.elev) == 15 | ...
%     qdoas_raw(:, no2_vis.elev) == 30 | ...
%     qdoas_raw(:, no2_vis.elev) == 90 | ...
%     qdoas_raw(:, no2_vis.az) == 287 | ...
%     qdoas_raw(:, no2_vis.az) == 355 | ...
%     qdoas_raw(:, no2_vis.az) == 45 | ...
%     qdoas_raw(:, no2_vis.az) == 95 | ...
%     qdoas_raw(:, no2_vis.az) == 145 | ...
%     qdoas_raw(:, no2_vis.az) == 195 | ...
%     qdoas_raw(:, no2_vis.az) == 245 );
% qdoas_raw = qdoas_raw(ind,:);

% % filter out HS, for specific day only
% ind= find(qdoas_raw(:, no2_vis.ft) < 11.676944 | ...
%     qdoas_raw(:, no2_vis.ft) > 11.748612 );
% qdoas_raw = qdoas_raw(ind,:);

% qdoas_raw = unique(qdoas_raw,'rows')
...
% tintfile=['/home/kristof/work/GBS/PEARL-GBS/CINDI-2_2016/data/PC1O_2016_' num2str(day) '.tint'];
...
% t_1int=tintarray(inds,3);
% scans=tintarray(inds,4);
% tint is ms in my files!!!
% calculate total integration time (tint) in seconds
% tint=qdoas_raw(:,scans).*qdoas_raw(:,t_1int)./1000;

% calculate total integration time (tint) in seconds
tint=qdoas_raw(:,scans).*qdoas_raw(:,t_1int);

% calculate relative intensity (ri) at given lambda (use exposure for
% single measurement here!)
ri_440=qdoas_raw(:,I_440)./qdoas_raw(:,t_1int);
ri_500=qdoas_raw(:,I_500)./qdoas_raw(:,t_1int);
...
for selection=[1,2,3]
    ...
    if selection==1 % NO2 and O4 vis
        ...
        colnum=24;
        f_out=['CINDI_files/UToronto_MAXDOAS_36_NO2vis_CINDI2_' date '_v1.asc'];
...
%         out_array(:,1)=day;
        ...
        fprintf(fid(1), '%s\n', '* Retrieval code: QDOAS (v2.109.4, August 2015)');
        ...
        fprintf(fid(1), '%s\n', '* Reference: averaged noon zenith spectrum (11:30-11:40 UTC)');
    ...
    elseif selection == 2 % NO2 and O4 alt
        ...
        % set array size and output file name
        colnum=24;
        f_out=['CINDI_files/UToronto_MAXDOAS_36_NO2visSmall_CINDI2_' date '_v1.asc'];
...
%         out_array(:,1)=day;
    ...
    elseif selection == 3 % O3
        
        
        % set array size and output file name
        colnum=27;
        f_out=['CINDI_files/UToronto_MAXDOAS_36_O3vis_CINDI2_' date '_v1.asc'];
...
%         out_array(:,1)=day;
        ...
        % DSCDs and errors
        out_array(:,8)=qdoas_raw(:,o3_vis.o3_dscd)./1e20;
        out_array(:,9)=qdoas_raw(:,o3_vis.o3_dscd_err)./1e20;

        out_array(:,10)=qdoas_raw(:,o3_vis.o3a_dscd)./1e20;
        out_array(:,11)=qdoas_raw(:,o3_vis.o3a_dscd_err)./1e20;

        out_array(:,12)=qdoas_raw(:,o3_vis.o4_dscd)./1e40;
        out_array(:,13)=qdoas_raw(:,o3_vis.o4_dscd_err)./1e40;

        out_array(:,14)=qdoas_raw(:,o3_vis.no2_dscd)./1e15;
        out_array(:,15)=qdoas_raw(:,o3_vis.no2_dscd_err)./1e15;

        out_array(:,16)=qdoas_raw(:,o3_vis.no2a_dscd)./1e15;
        out_array(:,17)=qdoas_raw(:,o3_vis.no2a_dscd_err)./1e15;

        out_array(:,18)=qdoas_raw(:,o3_vis.h2o_dscd)./1e23;
        out_array(:,19)=qdoas_raw(:,o3_vis.h2o_dscd_err)./1e23;
        
        out_array(:,20)=qdoas_raw(:,o3_vis.ring);
        out_array(:,21)=qdoas_raw(:,o3_vis.ring_err);

        % fit parameters and intensities
        out_array(:,22)=qdoas_raw(:,o3_vis.rms);
        out_array(:,23)=qdoas_raw(:,o3_vis.shift);
        out_array(:,24)=ri_500;
        out_array(:,25)=ci_3;
        out_array(:,26)=qdoas_raw(:,o3_vis.offset_0);
        out_array(:,27)=qdoas_raw(:,o3_vis.offset_1);
        ...
        % ... all QDOAS products for given retrieval (species+error)
        fprintf(fid(1), '%s\n', '* Y13-Axis (Col 8) = O3_DSCD_223 (1*10^20 molecules/cm2)');
        fprintf(fid(1), '%s\n', '* Y14-Axis (Col 9) = O3_DSCD_223_Error (1*10^20 molecules/cm2)');
        fprintf(fid(1), '%s\n', '* Y13-Axis (Col 10) = O3_DSCD_293 (1*10^20 molecules/cm2)');
        fprintf(fid(1), '%s\n', '* Y14-Axis (Col 11) = O3_DSCD_293_Error (1*10^20 molecules/cm2)');
        fprintf(fid(1), '%s\n', '* Y9-Axis (Col 12) = O4_DSCD (1*10^40 molec2/cm5)');
        fprintf(fid(1), '%s\n', '* Y10-Axis (Col 13) = O4_DSCD_Error (1*10^40 molec2/cm5)');
        fprintf(fid(1), '%s\n', '* Y7-Axis (Col  14) = NO2_DSCD_220 (1*10^15 molec/cm2)');
        fprintf(fid(1), '%s\n', '* Y8-Axis (Col  15) = NO2_DSCD_220_Error (1*10^15 molec/cm2)');
        fprintf(fid(1), '%s\n', '* Y11-Axis (Col 16) = NO2a_DSCD_298  (1*10^15 molec/cm2)  (Fit results for the "cold NO2 residue")');
        fprintf(fid(1), '%s\n', '* Y12-Axis (Col 17) = NO2a_DSCD_298_Error  (1*10^15 molec/cm2)');
        ...
        fprintf(fid(1), '%s\n', '* O3_293   : Serdyuchenko  et al., (2014), 293 K with I0 correction (1*10^20 molecules/cm2) pre-orthogonalized --> file: o3a_293p223K_SDY_450-550nm.xs');
        fprintf(fid(1), '%s\n', '* NO2_298  : Vandaele et al. (1998), 298 K with I0 correction (1*10^15 molecules/cm2) --> file: no2_298K_vanDaele.xs');
        fprintf(fid(1), '%s\n', '* NO2_220  : Vandaele et al. (1998), 220 K with I0 correction (1*10^15 molecules/cm2) pre-orthogonalized --> file: no2a_220p298K_vanDaele_450-500.xs');
        fprintf(fid(1), '%s\n', '*   O4     : Thalman and Volkamer 2013, 293 K  --> file: o4_thalman_volkamer_293K_air_corrected.xs ');
        fprintf(fid(1), '%s\n', '*   H2O    :  HITEMP,  Rothman  et  al.  (2010) -->  file:  H2O_HITEMP_2010_390-700_296K_1013mbar_air.xs  [in Hendrick et al., it was HITRAN 2004, Rothman et al., 2005]');
        fprintf(fid(1), '%s\n', '*      RING      : High  Resolution  calculation  with  QDOAS  according  to  Chance  and  Spurr  (1997)  and normalized as in Wagner et al. (2009) --> file: Ring_QDOAScalc_HighResSAO2010_Norm.xs ');

        % ... write column headers here
        head=sprintf('*DOY\tUTC\tTint\tSZA\tSAA\tElev\tViewing_angle\tO3_DSCD_223\tO3_DSCD_223_Error\tO3_DSCD_293\tO3_DSCD_293_Error\tO4_DSCD\tO4_DSCD_error\tNO2_DSCD_220\tNO2_DSCD_220_Error\tNO2a_DSCD_298\tNO2a_DSCD_298_error\tH2O_DSCD\tH2O_DSCD_Error\tRing\tRing_Error\tRMS\tSpectrum_shift\tIntens(500)\tCI(440/500)\toffset_cst\toffset_lin');
        fprintf(fid(1), '%s\n', head);
