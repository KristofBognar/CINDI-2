function Preprocs_CINDI(yyyy, jjj_beg, jjj_end, instrument, loc, ...
...
%
%%% ONLY WRITES L1 AND L2 FILES, NO .spe!!!
%
...
%                         line6_fin, line7, sp_l1, sp_l2);
                    write_files_hs_v2(filename, ind, line3_fin, line4_fin, line5_fin,...
                        line6_fin, line7, sp_l1, sp_l2);
...
%     fid(4) = fopen([filename '.tint'], 'w');% file to save MAX-DOAS integration times and n.o. scans
    ...
    for j = 1:length(ind)
        ...
        for k = 1:2
            ...
            %changed precision to 3 decimals
            line4 = line4_mat(i,:);
            fprintf(fid(k), '%3.3f\t %3.3f\t %3.3f\n', line4);
...
%%
function write_files_hs_v2(filename, ind,  line3_mat, line4_mat, line5_mat, line6_mat,...
    line7, sp_l1, sp_l2)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% same as write_files, but for horizon and almucantar scans, only saves
% .spe-like file, no L1 or L2
% file includes info about integration time as well (for elevation calibration study)
    
    disp(['Writing to ' filename])
    fid = [];
    fid(1) = fopen([filename '.asc'], 'w');
...
%         fprintf(fid(2), '%d\n', []);
        ...
        % write start time (hh, mm, ss), stop time, elev, az, t_exp and n.o. scans
        fprintf(fid(1), '%d %d %d %d %d %d %d %d %d %d ',...
            line3(1,10),line3(1,11),line3(1,12),...
            line3(1,16),line3(1,17),line3(1,18),...
            line6(1,2),line6(1,3),line5(1,8),line5(1,9));
